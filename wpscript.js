...
if ( true ) {
	...
	var settingsHighways = arguments[8];
	var settingsStreets = arguments[9];
	var settingsSmallStreets = arguments[10];
	var settingsBuildings = arguments[11];
	var settingsOres = arguments[12];
	var settingsNetherite = arguments[13];
	var settingsFarms = arguments[14];
	var settingsMeadows = arguments[15];
	var settingsQuarrys = arguments[16];
	var settingsAerodrome = arguments[17];
	var settingsMobSpawner = arguments[18];
	var settingsAnimalSpawner = arguments[19];
	var settingsRivers = arguments[20];
	var settingsStreams = arguments[21];
	var settingsVolcanos = arguments[22];
	var settingsShrubs = arguments[23];
	var settingsCrops = arguments[24];
	var settingsMapVersion = arguments[25];
	var settingsMapOffset = arguments[26];
	var settingsVanillaPopulation = arguments[27];
	var heightmapName = arguments[28];
	var biomeSource = arguments[29];
	var mod_BOP = arguments[30];
	var mod_BYG = arguments[31];
	var mod_Terralith = arguments[32];
	var mod_Create = arguments[33];
}
...
if ( true ) {
	...
	if ( settingsMapVersion === "1-16" || settingsMapVersion === "1-17" || settingsMapVersion === "1-18" || settingsMapVersion === "1-19" ) {
	...
	if ( settingsMapVersion === "1-18" || settingsMapVersion === "1-19" ) {
		var deepslateCoalOreLayer = wp.getLayer().fromFile(path+'wpscript/ores/coal_ore_deepslate.layer').go();
		var deepslateCoalDepositLayer = wp.getLayer().fromFile(path+'wpscript/ores/coal_deposit_deepslate.layer').go();
...
if ( true ) {
	...
	if ( settingsMapVersion === "1-12" ) {
		...
		var world2 = wp.getWorld()
			...
			.go();
		var platformMapFormat = world2.getPlatform();
	} else if ( settingsMapVersion === "1-18" && tilesPerMap === 1 && ( verticalScale === 5 || verticalScale === 10 || verticalScale === 25 ) ) {
		var world2 = wp.getWorld()
			.fromFile(path+'wpscript/1-18-ex.world') //ex has -64/2032 block limit (only usefull for 1 DPT)
			.go();
		var platformMapFormat = world2.getPlatform();
	} else if ( settingsMapVersion === "1-19" && tilesPerMap === 1 && ( verticalScale === 5 || verticalScale === 10 || verticalScale === 25 ) ) {
		var world2 = wp.getWorld()
		...
		var world2 = wp.getWorld()
			...
			.go();
		var platformMapFormat = world2.getPlatform();
	} else if ( settingsMapVersion === "1-19" ) {
		var world2 = wp.getWorld()
	...
	if (verticalScale === 200) {
		var world = wp.createWorld()
			...
			.fromLevels(0, 255).toLevels(62, 106)
		...
		var world = wp.createWorld()
			.fromHeightMap(heightMap)
			.shift(shiftLongitute, shiftLatitude)
			.fromLevels(0, 255).toLevels(62, 151)
			.withMapFormat(platformMapFormat)
			.go();
		...
		var world = wp.createWorld()
			.fromHeightMap(heightMap)
			.shift(shiftLongitute, shiftLatitude)
			.fromLevels(0, 255).toLevels(62, 239)
			.withMapFormat(platformMapFormat)
			.go();
		...
		if ( settingsMapVersion === "1-18" || settingsMapVersion === "1-19" ) {
			var world = wp.createWorld()
				.fromHeightMap(heightMap)
				.shift(shiftLongitute, shiftLatitude)
				.fromLevels(0, 255).toLevels(62, 317) //new height only on 1.18+ maps
				.withMapFormat(platformMapFormat)
				.go();
		} else {
			var world = wp.createWorld()
				.fromHeightMap(heightMap)
				.shift(shiftLongitute, shiftLatitude)
				.fromLevels(0, 204).toLevels(62, 254)
				.withMapFormat(platformMapFormat)
				.go();
		}
	} else {
	...
	if ( new java.io.File(path+'image_exports/'+tile+'/heightmap/'+heightmapName+'.png').isFile() ) {
		...
		if ( verticalScale === 200 ) {
			var world = wp.createWorld()
				.fromHeightMap(heightMap)
				.shift(shiftLongitute, shiftLatitude)
				.fromLevels(0, 50).toLevels(56, 106)
				.withMapFormat(platformMapFormat)
				.go();
		} else if ( verticalScale === 100 ) {
			var world = wp.createWorld()
				.fromHeightMap(heightMap)
				.shift(shiftLongitute, shiftLatitude)
				.fromLevels(0, 100).toLevels(50, 150)
				.withMapFormat(platformMapFormat)
				.go();
		} else if ( verticalScale === 75 ) {
			var world = wp.createWorld()
				.fromHeightMap(heightMap)
				.shift(shiftLongitute, shiftLatitude)
				.fromLevels(0, 133).toLevels(47, 180)
				.withMapFormat(platformMapFormat)
				.go();
		} else if ( verticalScale === 50 ) {
			var world = wp.createWorld()
				.fromHeightMap(heightMap)
				.shift(shiftLongitute, shiftLatitude)
				.fromLevels(0, 200).toLevels(39, 239)
				.withMapFormat(platformMapFormat)
				.go();
		} else if ( verticalScale === 35 && ( settingsMapVersion === "1-18" || settingsMapVersion === "1-19" ) ) {
			var world = wp.createWorld()
				.fromHeightMap(heightMap)
				.shift(shiftLongitute, shiftLatitude)
				.fromLevels(0, 285).toLevels(29, 315)
				.withMapFormat(platformMapFormat)
				.go();
		} else if ( verticalScale === 25 && ( settingsMapVersion === "1-18" || settingsMapVersion === "1-19" ) ) {
			var world = wp.createWorld()
				.fromHeightMap(heightMap)
				.shift(shiftLongitute, shiftLatitude)
				.fromLevels(0, 400).toLevels(16, 416)
				.withMapFormat(platformMapFormat)
				.go();
		} else if ( verticalScale === 10 && ( settingsMapVersion === "1-18" || settingsMapVersion === "1-19" ) ) {
			var world = wp.createWorld()
				.fromHeightMap(heightMap)
				.shift(shiftLongitute, shiftLatitude)
				.fromLevels(0, 1000).toLevels(-53, 947)
				.withMapFormat(platformMapFormat)
				.go();
		} else if ( verticalScale === 5 && ( settingsMapVersion === "1-18" || settingsMapVersion === "1-19" ) ) {
			var world = wp.createWorld()
				.fromHeightMap(heightMap)
				.shift(shiftLongitute, shiftLatitude)
				.fromLevels(0, 2000).toLevels(-138, 1832)
				.withMapFormat(platformMapFormat)
				.go();
		}
	}
...
if ( true ) {
	...
	var belowDagreesForRivers = wp.createFilter()
		.belowDegrees(35)
		.go();
...
//Insert custom biomes / modded biomes beginning here:
   ...
   wp.applyHeightMap(ecoRegionImage)
    ...
    .fromColour(61,167,238).toLevel(BIOME_FOREST) //Nihonkai evergreen forests
    .fromColour(206,145,65).toLevel(BIOME_WINDSWEPT_FOREST) //Nihonkai montane deciduous forests
   ...
   wp.applyHeightMap(ecoRegionImage)
    ...
    .fromColour(216,79,77).toLevel(BIOME_FROZEN_PEAKS) //Rock and Ice
    ...
    .fromColour(146,66,217).toLevel(BIOME_FOREST) //Taiheiyo evergreen forests
...
if ( settingsVanillaPopulation === "False" ) {
	...
	var spruceImage = wp.getHeightMap().fromFile(path+'image_exports/'+tile+'/'+tile+'_wither_rose.png').go();
	wp.applyHeightMap(spruceImage) 
		.toWorld(world)
		.shift(shiftLongitute, shiftLatitude)
...
if ( true ) {
	...
	wp.applyHeightMap(riverImage) 
		...
		.shift(shiftLongitute, shiftLatitude)
		.applyToLayer(borderLayer)
		.withFilter(belowDagreesForRivers)
		.fromLevels(0, 230).toLevel(0)
		.go();
	wp.applyHeightMap(wetImage) 
		.toWorld(world)
...
if ( settingsOres === "True" ) {
	...
	if ( settingsMapVersion === "1-18" || settingsMapVersion === "1-19" ) {	
		wp.applyHeightMap(coalImage) 
			.toWorld(world)
			.shift(shiftLongitute, shiftLatitude)
			.applyToLayer(deepslateCoalDepositLayer)
			.fromLevels(0, 15).toLevel(8)
			.fromLevels(16, 31).toLevel(7)
			.fromLevels(32, 47).toLevel(7)
			.fromLevels(48, 63).toLevel(6)
	...
	if ( ( settingsMapVersion === "1-16" || settingsMapVersion === "1-17" || settingsMapVersion === "1-18" || settingsMapVersion === "1-19" ) && settingsNetherite === "True" ) {
...
if ( settingsVanillaPopulation === "True" && ( settingsMapVersion === "1-12" || settingsMapVersion === "1-18" || settingsMapVersion === "1-19" ) ) {
